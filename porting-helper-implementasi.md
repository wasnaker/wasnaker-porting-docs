...
| 11 | **Ekstraksi core generik → package `spine/laravel-spine`** (repo `laravelspine/laravelspine`; namespace `Spine\`; src/ + routes + migrations; provider auto-load routes+migrations) | ✅ Port | ADA (via app konsumen) | wasnaker-core jadi konsumen (app ramping: auth/user/tenant) + konsumen dev `spine.lan`; terminologi Perfex/wasnaker dibersihkan; `User` TIDAK di package (resolve via `config('auth.providers.users.model')`); fix bug lama gdpr (user_id dari request, bukan arg route) |
